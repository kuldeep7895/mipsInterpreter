...
int main() {
    ...
    while (i < tokens.size()) {
        ...
        if (tokens[i].size() != 0) {
            ...
            //cout<<clockRem<<"rex"<<endl;

            if (true) {

                if (checkDependence && !first && (regUpdateLoc.size() > 0 || curMemUpdateLoc.size() > 0)) {

			
                    cout << "clock " << parallelClock << " : ";
                    cout<<"Instruction : "<<lines[i]<<endl;
                    ...
                    cout << endl<<endl;
                } else if (regUpdateLoc.size() > 0 || curMemUpdateLoc.size() > 0) {
            ...
            if (tokens[i] == allTokens[1]) {
                ...
                if (reg[reg2] == INT_MAX || reg[reg3] == INT_MAX) {
                    ...
                    if (clockRem > 0 && (dependentReg != reg1) && (dependentReg != reg2) && (dependentReg != reg3)) {
                        clockRem--;

                    } else if (clockRem != -1) {
                        clockRem = -1;
                        checkDependence = false;
            ...
            // LW REG NUM ( REG )
                ...
                if (reg[reg2] == INT_MAX) {
                    ...
                    if (ROW_BUFFER == 0) {
                        ...
                        cout << "clock " << ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << ROW_ACCESS_DELAY + countClockCycle + COL_ACCESS_DELAY + 1 << " loaded data row buffer at word address "<<loc<< " to register " << registerNames[reg1] << endl;
                        ...
                        if (ROW_BUFFER == row) {
                            ...
                            cout << "clock " << countClockCycle + 2 << " to " << countClockCycle + COL_ACCESS_DELAY + 1 << " loaded data row buffer at word address "<<loc<< " to register " << registerNames[reg1] << endl;
                         ...
                         countRowBufferUpdate += 1;
                            cout << "clock " << countClockCycle + 2 << " to " << ROW_ACCESS_DELAY + countClockCycle + 1 << " transferred buffer row to main memory row" << ROW_BUFFER << endl;

                            cout << "clock " << ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << 2 * ROW_ACCESS_DELAY + countClockCycle + 1 << " updated row buffer with main memory row " << row << endl;
                            cout << "clock " << 2 * ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << 2 * ROW_ACCESS_DELAY + countClockCycle + COL_ACCESS_DELAY + 1 << " row buffer to register " << registerNames[reg1] << endl;
            ...
            // LW REG NUM 
                ...
                if (ROW_BUFFER == 0) {
                    ...
                    if (ROW_BUFFER == row) {
                        ...
                        cout << "clock " << ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << 2 * ROW_ACCESS_DELAY + countClockCycle + 1 << " updated row buffer with main memory row " << row << endl;
                        cout << "clock " << 2 * ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << 2 * ROW_ACCESS_DELAY + countClockCycle + COL_ACCESS_DELAY + 1 << " loaded data row buffer at word address "<<loc<< " to register " << registerNames[reg1] << endl;
                        ROW_BUFFER = row;
                        countRowBufferUpdate += 1;
                        countClockCycle += (ROW_ACCESS_DELAY * 2) + COL_ACCESS_DELAY + 1;
                        clockRem = (ROW_ACCESS_DELAY * 2) + COL_ACCESS_DELAY;
            ...
            // LW REG (REG)
                ...
                if (reg[reg2] == INT_MAX) {
                    ...
                    if (ROW_BUFFER == 0) {
                        countRowBufferUpdate += 1;
                        cout << "clock " << countClockCycle + 2 << " to " << ROW_ACCESS_DELAY + countClockCycle + 1 << " activated row " << row << endl;
                        cout << "clock " << ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << ROW_ACCESS_DELAY + countClockCycle + COL_ACCESS_DELAY + 1 << " copied data from row buffer to register " << registerNames[reg1] << endl;
                        ROW_BUFFER = row;
                        countClockCycle += ROW_ACCESS_DELAY + COL_ACCESS_DELAY + 1;
                        clockRem = (ROW_ACCESS_DELAY) + COL_ACCESS_DELAY;
                    } else {
            ...
            // SW REG NUM ( REG )
                ...
                if (reg[reg2] == INT_MAX || reg[reg1] == INT_MAX) {
                    ...
                    if (ROW_BUFFER == 0) {
                        ...
                        cout << "clock " << countClockCycle + 2 << " to " << ROW_ACCESS_DELAY + countClockCycle + 1 << " activated row " << row << endl;
                        cout << "clock " << ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << ROW_ACCESS_DELAY + countClockCycle + COL_ACCESS_DELAY + 1 << " copied data from register "<<registerNames[reg1]<<" to row buffer at word loc " << loc<<endl;
                        ROW_BUFFER = row;
                        countClockCycle += ROW_ACCESS_DELAY + COL_ACCESS_DELAY + 1;
                        clockRem = (ROW_ACCESS_DELAY) + COL_ACCESS_DELAY;
                    } else {
                        if (ROW_BUFFER == row) {
                            countRowBufferUpdate += 1;
                            cout << "clock " << countClockCycle + 2 << " to " << countClockCycle + COL_ACCESS_DELAY + 1 << " copied data from register "<<registerNames[reg1]<<" to row buffer at word loc " << loc<<endl;
                            countClockCycle += COL_ACCESS_DELAY + 1;
                            clockRem = COL_ACCESS_DELAY;
                        } else {
                            countRowBufferUpdate += 2;
                            ...
                            cout << "clock " << ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << 2 * ROW_ACCESS_DELAY + countClockCycle + 1 << " updated row buffer with main memory row " << row << endl;
                            cout << "clock " << 2 * ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << 2 * ROW_ACCESS_DELAY + countClockCycle + COL_ACCESS_DELAY + 1 << " copied data from register "<<registerNames[reg1]<<" to row buffer at word loc " << loc<<endl;
                            ROW_BUFFER = row;
                            countClockCycle += (ROW_ACCESS_DELAY * 2) + COL_ACCESS_DELAY + 1;
                            clockRem = (ROW_ACCESS_DELAY * 2) + COL_ACCESS_DELAY;
                        }
		...
		stored = false;
            ...
            // ADDI REG1 REG2 NUM
                ...
                if (reg[reg2] == INT_MAX) {
                    ...
                    if (clockRem > 0 && (dependentReg != reg1) && (dependentReg != reg2)) {
                        clockRem--;
                ...
                if (reg[reg2] == INT_MAX || reg[reg3] == INT_MAX) {
                    ...
                    if (ROW_BUFFER == 0) {
                        ...
                        if (ROW_BUFFER == row) {
                            ...
                            cout << "clock " << ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << 2 * ROW_ACCESS_DELAY + countClockCycle + 1 << " updated row buffer with main memory row " << row << endl;
                            cout << "clock " << 2 * ROW_ACCESS_DELAY + countClockCycle + 2 << " to " << 2 * ROW_ACCESS_DELAY + countClockCycle + COL_ACCESS_DELAY + 1 << " copied data from row  buffer to register " << registerNames[reg1] << endl;
                            ROW_BUFFER = row;
                            countClockCycle += (ROW_ACCESS_DELAY * 2) + COL_ACCESS_DELAY + 1;
                            clockRem = (ROW_ACCESS_DELAY * 2) + COL_ACCESS_DELAY;
                        }
...
cout<<endl<<endl;
    ...
    if(!stored){
  	countClockCycle = countClockCycle+ROW_ACCESS_DELAY;
  	cout<<"Stored unstored buffer row to main memory"<<endl;
  }
    
	cout<<endl;
    cout << "Words stored " << countWords << endl<<endl;
    cout << "Clock cycle count = " << countClockCycle << endl<<endl;
    cout << "Number of row buffer updates =" << countRowBufferUpdate << endl<<endl;
    cout << "Final data values that are updated during execution:" << endl<<endl;
